...
var renderUnitPrice = (unitPrice, unitPriceMeasurement) => {
  if (unitPrice && unitPriceMeasurement) {
    ...
    return '\n      <div class="unit-price '
      ...
      .concat(
        strings$5.product.unitPriceSeparator,
...
var { strings: strings$3 } = window.theme;
var selectors$A = {
...
var ShippingEstimator = (node, container) => {
  ...
  if (firstCountryOptions.length > 1) {
    ...
    firstCountryOptions[0].innerText =
      strings$3.products.product.country_placeholder;
  }
        ...
        request.onload = () => {
          ...
          if (request.status >= 200 && request.status < 300) {
            if (response.shipping_rates && response.shipping_rates.length) {
              ...
              var noRate =
                '\n                <li class="shipping-estimator-modal__success-item">\n                  <span>'.concat(
                  strings$3.products.product.no_shipping_rates,
                  "</span>\n                </li>\n              "
                );
  ...
  function getSearchResults(searchTerm) {
    ...
    fetch(
      ""
        ...
        .concat(encodeURIComponent(searchTerm), "&section_id=predictive-search")
    )
      .then((response) => {
        if (!response.ok) {
      ...
      .then((text) => {
        var resultsMarkup = new DOMParser()
          ...
          .querySelector("#shopify-section-predictive-search").innerHTML; // Cache results
